...
MARKER = ".qtdeploykit"  # 绿色目录标记,只替换带此标记的目录
...
def load_config(path: Path) -> dict:
    ...
    if (Path(app_name).name != app_name
            ...
            or app_name.rstrip(" .") != app_name):
        raise DeployError("[app].name 将用作绿色目录名,不能包含路径或 Windows 非法字符")
...
def prepare_dist(out_dir: Path, directory_name: str) -> Path:
    """Create a private staging directory without touching the last good output."""
    out_dir.mkdir(parents=True, exist_ok=True)
    dist = out_dir / directory_name
    if dist.exists() and not (dist / MARKER).is_file():
        raise DeployError(
            f"输出目录已存在且不是本工具生成的(缺 {MARKER} 标记),拒绝替换:{dist}\n"
            "请手动确认后删除,或在配置里换一个 output.dir")

    staging = out_dir / f".qtdeploykit-staging-{uuid.uuid4().hex}"
    staging.mkdir()
    (staging / MARKER).write_text("generated by QtDeployKit\n", encoding="utf-8")
...
def commit_dist(staging: Path, out_dir: Path, directory_name: str) -> Path:
    """Atomically publish staging, restoring the previous output on failure."""
    dist = out_dir / directory_name
    backup = out_dir / f".qtdeploykit-backup-{uuid.uuid4().hex}"
    had_old = dist.exists()
    try:
        if had_old:
            dist.replace(backup)
        ...
        if had_old and backup.exists() and not dist.exists():
            backup.replace(dist)
        raise DeployError(f"发布新的绿色目录失败:{e}") from e
    if backup.exists():
        try:
            shutil.rmtree(backup)
        except OSError as e:
            log(f"      警告:旧绿色目录备份清理失败,可手动删除 {backup}: {e}")
    return dist
...
def copy_extra_files(cfg: dict, dist: Path) -> list[str]:
    ...
    for src, dest in cfg["extra_files"]:
        if isinstance(src, tuple):  # (base, pattern) 通配
            ...
            matches = [src]
        target_dir = (dist / dest).resolve() if dest else dist.resolve()
        try:
            target_dir.relative_to(dist.resolve())
        except ValueError:
            raise DeployError(
                f"extra_files 的 dest 不能指向绿色目录之外:{dest}")
...
def scan_and_complete(dist: Path, search_dirs: list[Path], want_machine: str,
    ...
    queue = [p for p in sorted(dist.rglob("*"))
             ...
             and not (p.parent == dist and p.name.lower() in payload)]
    scanned: set[str] = set()

    def locate_dependency(pe: Path, name: str) -> Path | None:
        # Windows first searches the importing module's directory; application
        # dependencies and most Qt plugin dependencies can then live at root.
        for candidate in (pe.parent / name, dist / name):
    ...
    while queue:
        pe = queue.pop(0)
        key = pe.relative_to(dist).as_posix().lower()
        if key in scanned:
            continue
        scanned.add(key)
        ...
        if machine != want_machine:
            ...
            continue
        for dep in imports:
            local_dep = locate_dependency(pe, dep)
            if local_dep:
                dep_key = local_dep.relative_to(dist).as_posix().lower()
                if local_dep.parent == dist and dep in payload and dep_key not in scanned:
...
def main() -> None:
    ...
    t = time.perf_counter()
    dist = prepare_dist(cfg["out_dir"], cfg["name"])
    ...
    if not cfg["qml_dirs"] and any((dist / n).exists() for n in ("Qt5Qml.dll", "Qt6Qml.dll")):
        raise DeployError(
            "程序链接了 QML(绿色目录里有 QtQml.dll),但配置没给 [qt].qml_dirs,\n"
    ...
    log(f"      绿色目录共 {n_files} 个文件、{fmt_size(total_size)}"
    ...
    # The generated script should remain reusable after the staging directory
    # has been renamed to its public location. Generate all side artifacts
    # before publishing so replacing the app directory remains the final fallible step.
    if not args.skip_installer:
        generate_iss(cfg, version, cfg["out_dir"] / cfg["name"], iscc, machine,
                     vc_path if vc_mode == "installer" else None)
    notes = write_notes(cfg, version, report, extra, setup, vc_desc)
    final_dist = commit_dist(dist, cfg["out_dir"], cfg["name"])
    log("")
    log(f"完成(总耗时 {time.perf_counter() - t_total:.0f} 秒):")
    log(f"  绿色目录   {final_dist}")
    if setup:
