...
def scan_and_complete(dist: Path, search_dirs: list[Path], want_machine: str) -> dict:
    """递归扫描 dist 内所有 PE 的导入表;缺的 DLL 先在 search_dirs 找并自动拷入,
    找不到且不是系统件/VC 运行时的记为 missing。返回分类报告。"""
    report = {"copied": [], "vc_runtime": set(), "missing": {}, "arch_mismatch": []}
    queue = [p for p in sorted(dist.rglob("*")) if p.suffix.lower() in (".dll", ".exe")]
    scanned: set[str] = set()
...
def generate_iss(cfg: dict, version: str, dist: Path, iscc: Path, machine: str) -> Path:
    tpl_path = Path(__file__).parent / "templates" / "installer.iss.tpl"
    tpl = tpl_path.read_text(encoding="utf-8")

    out_dir = cfg["out_dir"]
    installer_name = cfg["installer_name"] or f"{cfg['name']}-{version}-setup"
    ...
    if cfg["vc_redist"]:
        if not cfg["vc_redist"].is_file():
            raise DeployError(f"[installer].vc_redist 指定的文件不存在:{cfg['vc_redist']}")
        vc_file_line = f'Source: "{cfg["vc_redist"]}"; DestDir: "{{tmp}}"; Flags: deleteafterinstall'
        vc_run_line = ('Filename: "{tmp}\\' + cfg["vc_redist"].name + '"; '
...
def run_iscc(iscc: Path, iss: Path) -> Path:
    log(f"  $ {iscc} {iss}")
    ...
    setups = sorted(out_dir.glob("*.exe"), key=lambda p: p.stat().st_mtime)
    if not setups:
        raise DeployError(f"ISCC 成功但在 {out_dir} 没找到安装包")
    return setups[-1]
...
def write_notes(cfg: dict, version: str, report: dict, extra: list[str],
    ...
    redist = str(cfg["vc_redist"]) if cfg["vc_redist"] else "未内嵌(目标机需自行安装,见下)"
...
本包不散拷 vcruntime/msvcp DLL。目标机需要 Microsoft Visual C++ 2015-2022
Redistributable (x64)。若安装包未内嵌,请在配置 [installer].vc_redist 指向
vc_redist.x64.exe(官方下载:https://aka.ms/vs/17/release/vc_redist.x64.exe ),
安装时会静默安装、已装则跳过。缺它的典型症状就是 0xc000012f / 0xc000007b
或「找不到 VCRUNTIME140.dll」。
...
def main() -> None:
    ...
    log("[4/6] 依赖闭包扫描")
    report = scan_and_complete(dist, cfg["search_dirs"], machine)
    ...
    if report["missing"]:
        ...
        raise DeployError(
            ...
            + "\n".join(lines))
    if report["vc_runtime"] and not cfg["vc_redist"] and not args.skip_installer:
    ...
    if args.skip_installer:
        ...
        log("[6/6] 生成安装包")
        iss = generate_iss(cfg, version, dist, iscc, machine)
        setup = run_iscc(iscc, iss)
        if cfg["sign_enabled"]:
            sign_file(cfg, setup)

    notes = write_notes(cfg, version, report, extra, setup)
    log("")
    log("完成:")
    log(f"  绿色目录   {dist}")
    if setup:
